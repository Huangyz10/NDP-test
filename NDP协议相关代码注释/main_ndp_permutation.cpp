...
#include "fat_tree_topology.h"//调用胖树结构
...
uint32_t RTT = 1; // 这是us级的每条链路的延迟; 普遍的RTT=0.02ms
#define DEFAULT_NODES 128//定义默认节点数128个
#define DEFAULT_QUEUE_SIZE 8//定义默认队列长度8

FirstFit* ff = NULL;
unsigned int subflow_count = 1;//无符号定义整形subflow_count为1
...
EventList eventlist;//创建驱动htsim模拟器的类
...
int main(int argc, char **argv) {
    Packet::set_packet_size(9000);//设置数据包大小：9000
    eventlist.setEndtime(timeFromSec(0.201));//设置总的模拟时间
    ...
    logfile.setStartTime(timeFromSec(0));//设置记录开始时间
    NdpSinkLoggerSampling sinkLogger = NdpSinkLoggerSampling(timeFromMs(10), eventlist);//迭代所有的NdpSinks并且每10ms记录他们的速率
    ...
    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, queuesize, 
					       &logfile, &eventlist,ff,COMPOSITE,0);//生成胖树拓扑
#endif
    ...
    //初始化所有的sources/sinks
    NdpSrc::setMinRTO(50000); //增加RTO来避免假的中继
